...
public:
    ...
    TV evaluateSpringForce(int idx, std::vector<TV>& dx) {
        ...
        TV curSpring = xA - xB;
        TV curForce =  -(youngs_modulus * (curSpring.norm() - rest_length.at(idx)) / rest_length.at(idx)) * curSpring.normalized() ;
    ...
    TV evaluateDampingForce(int idx, std::vector<TV>& dx, T dt) {
        ...
        T relSpeed = (vA - vB).dot(n);
        TV curForce = -damping_coeff * relSpeed * n;
    ...
    TM evaluateKS(int idx, std::vector<TV>& dx, bool project_pd=true) {
        ...
        TV curSpring = xA - xB;
        T l0 = rest_length.at(idx);
        T l = curSpring.norm();

        TV n = curSpring.normalized();
        TM nnTrans = n * n.transpose();

        KS = -(youngs_modulus * ( 1.0f / l0 - 1.0f / l) * (TM::Identity() - nnTrans) + youngs_modulus * nnTrans / l0); 
    ...
    TM evaluateKD(int idx, T dt) {
        ...
        TV n = (x[A] - x[B]).normalized();
        TM KD = -damping_coeff * (n * n.transpose()) / dt;
